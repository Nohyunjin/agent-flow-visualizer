...
시작 화면은 **Dashboard**입니다. 기본으로 각 에이전트의 가장 느린 종료 턴 순서로 정렬합니다. `d`로 Dashboard와 기존 Flow 탐색을 전환할 수 있습니다. Flow에서 바로 시작하려면 `agent-flow --flow`를 사용하세요.
...
- `o`: 가장 느린 종료 턴 → 누적 종료 턴 시간 → 가장 느린 도구 → 미완료 턴 경과 시간 순으로 정렬 기준을 바꿉니다.
...
새 로그로 순위가 달라져도 선택한 세션과 턴을 유지합니다. Dashboard는 125열 이상에서 두 목록을 나란히, 95~124열의 충분히 높은 창에서는 위아래로, 작은 창에서는 현재 목록만 표시합니다.
