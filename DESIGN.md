...
When any loaded descendant is WORKING, emphasize its ancestor's title in yellow and bold, with `● N sub working` first on the fourth metadata line so narrow panes keep the count visible. Include nested and filtered-out descendants; keep the parent's own status unchanged. Resolve activity at render time using the existing transcript status rules, so completion or aging into QUIET removes the emphasis. Keep four-line rows and stable text rather than blinking titles.
